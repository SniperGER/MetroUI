	...
	var MetroUI = {
		...
		create: function(options) {
			...
			if (type=="list") {
				$('#'+id).addClass("metro list");
				$('#'+id).bind(touchEventStart, function() {
					if (listActivated) {
						$('#'+id).addClass("accentBorder "+currentAccentColor);
					}
				})
				$('#'+id).bind(touchEventEnd, function() {
					if (listActivated) {
						$('#'+id).addClass("extended").css("height",($('#'+id+' p').length * 42)+"px");
						$('#'+id+' p:nth-child(1)').removeAttr("style");
						$('#'+id+' p:nth-child('+listSelectedIndex+')').addClass("accentColor "+currentAccentColor);
						timeouts.push(setTimeout(function() {
							listActivated = false;
						}, 200) );
					}
				});
				$('#'+id+' p').bind(touchEventEnd, function() {
					if (!listActivated) {
						var paddingIndex = -Math.abs($(this).index() * 32);
						listSelectedIndex = $(this).index()+1;
						$('#'+id+' p').removeClass("accentColor "+currentAccentColor);
						$('#'+id).removeClass("extended accentBorder "+currentAccentColor).removeAttr("style")
						$('#'+id+' p:nth-child(1)').css("margin-top", paddingIndex+"px");
						timeouts.push(setTimeout(function() {
							listActivated = true;
							setTimeout(callback, 5);
						}, 200) );
